...
from app.models.entities import TipResult
...
@pytest.mark.parametrize(
    ...
    [
        ("home_win", "1", 2, 0, TipResult.WON),
        ("home_win", "1", 1, 1, TipResult.LOST),
        ("away_win", "2", 0, 3, TipResult.WON),
        ("draws", "X", 1, 1, TipResult.WON),
        ("double_chance", "1X", 1, 1, TipResult.WON),
        ("double_chance", "1X", 0, 1, TipResult.LOST),
        ("double_chance", "X2", 0, 0, TipResult.WON),
        ("btts", "GG", 1, 1, TipResult.WON),
        ("btts", "GG", 3, 0, TipResult.LOST),
        ("over_1_5", "Over 1.5", 1, 1, TipResult.WON),
        ("over_1_5", "Over 1.5", 1, 0, TipResult.LOST),
        ("over_2_5", "Over 2.5", 2, 1, TipResult.WON),
        ("under_3_5", "Under 3.5", 2, 1, TipResult.WON),
        ("under_3_5", "Under 3.5", 2, 2, TipResult.LOST),
        ("unknown_market", "?", 1, 0, TipResult.VOID),
    ],
)
def test_settlement_rules(market, selection, hg, ag, expected) -> None:
    assert settle_selection(market, selection, hg, ag) is expected
