...
from app.models.entities import League, Match, PublishedTip, Team, TipResult
...
def test_settlement_scores_finished_matches(db_session: Session) -> None:
    ...
    assert db_session.query(PublishedTip).one().result is TipResult.WON
...
def test_unfinished_matches_stay_pending(db_session: Session) -> None:
    ...
    assert db_session.query(PublishedTip).one().result is TipResult.PENDING
...
def test_performance_computes_strike_rate_and_roi(db_session: Session) -> None:
    ...
    for i, (result, odds) in enumerate(
        [(TipResult.WON, 1.8), (TipResult.WON, 1.8), (TipResult.LOST, 1.8), (TipResult.LOST, 1.8)]
    ):
