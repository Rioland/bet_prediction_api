...
from app.models.entities import Match, PublishedTip, TipResult
from app.services.ingest import FINISHED_STATUSES


def settle_selection(market: str, selection: str, home_goals: int, away_goals: int) -> TipResult:
    ...
    return TipResult.WON if won else TipResult.LOST
...
def settle_pending_tips(db: Session) -> dict[str, int]:
    ...
    pending = db.scalars(
        ...
        .where(
            PublishedTip.result == TipResult.PENDING,
...
def performance(db: Session, days: int = 30, market: str | None = None) -> dict[str, Any]:
    ...
    stmt = select(PublishedTip).where(
        ...
        PublishedTip.result.in_([TipResult.WON, TipResult.LOST]),
    )
    if market:
    ...
    won = [t for t in tips if t.result == TipResult.WON]
...
def performance_by_market(db: Session, days: int = 30) -> list[dict[str, Any]]:
    markets = db.scalars(
        ...
        .where(PublishedTip.result.in_([TipResult.WON, TipResult.LOST]))
...
def recent_settled(db: Session, limit: int = 20) -> list[PublishedTip]:
    return list(
        db.scalars(
            select(PublishedTip)
            .where(PublishedTip.result != TipResult.PENDING)
